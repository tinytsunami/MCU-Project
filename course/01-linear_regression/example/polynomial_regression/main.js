(function(){
  ...
  let refresh = function(draw_test){
    ...
    for(let i = 0; i < SIZE; i++){
      drawLine(i, h(i, theta), i+1, h(i+1, theta), LINE_COLOR);
    }
  };
  ...
  let lambda = 100; //regularized
  let alpha = 5e-10; //lreaning rate
  let theta = [0, 0, 0, 0]; //weight

  //hypothesis function
  let h = function(x, theta){
    return (theta[0] + theta[1] * Math.sqrt(x) + theta[2] * x + theta[3] * Math.pow(x, 2));
  };
  ...
  let dJ = function(x, y, theta){
    ...
    for(let i in x){
      ...
      tmp[1] += (h(x[i], theta) - y[i]) * Math.sqrt(x[i]);
      tmp[2] += (h(x[i], theta) - y[i]) * x[i];
      tmp[3] += (h(x[i], theta) - y[i]) * Math.pow(x[i], 2);
    }
    for(let i = 0; i < 4; i++){
      tmp[i] /= x.length;
    }
  ...
  let process_train = function(){
    ...
    for(let i = 0; i < 4; i++){
      theta[i] = (theta[i] * (1 - alpha * lambda / x.length) - alpha * grad[i]);
    }
